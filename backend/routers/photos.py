...
async def get_random_photos(limit: int = 10):
    try:
...
@router.get("/business/{photo_id}", response_model=str)
async def get_photo_business(photo_id: str):
    try:
        photo = await Photo.find_one(Photo.photo_id == photo_id)
        if not photo:
            raise HTTPException(status_code=404, detail="Photo not found")
        return photo.business_id
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/by-business/{business_id}", response_model=List[Photo])
